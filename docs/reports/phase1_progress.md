...
- Task 037 Stage 0：冻结 Current FrFT，修正 Task 036-fix2 历史同步状态和旧 runner 默认目录。
- Task 037 Stage 1：旧 chirp-convolution FrFT 的能量/inverse 审计失败；完成最小核心修正为有限维 centered unitary DFT 的谱分数幂，92/92 性质测试、5/5 边界测试、Oracle audit 全部通过。

## In Progress

- Task 037：Stage 0–7 已完成；最终 `REJECTED_NO_USABLE_SEPARABILITY`。FrFT 仅保留 baseline，candidate matrix/RL 均 false，Task 038 未启动。
...
- Task 037 Stage 7：回归通过（Interface 800/800、correctness failures 0），完成最终报告，等待 record/push 状态收口。
