...
from anti_jamming.adapt_filter_fair import apply_adapt_filter_fair, fit_adapt_filter_fair  # noqa: E402
...
def load_frozen_candidate() -> tuple[dict, str]:
    path = Path('results/phase1/task036_fix/stageD/selected_candidate.json')
    payload = json.loads(path.read_text())
    if payload.get('selected_candidate') is not None:
        raise RuntimeError('Stage E expects rejection-confirmation-only mode, not a selected candidate')
    candidate = payload.get('rejection_confirmation_candidate')
    if not candidate:
        raise RuntimeError('Stage D did not freeze a rejection-confirmation representative')
...
def run(output_dir: Path) -> dict:
    ...
    candidate, candidate_hash = load_frozen_candidate()
    candidate_source_commit = 'be8fa2a'
    rows = []
    oracle_target_cache = {}
    fair_target_cache = {}
            ...
            for seed in SEEDS:
                ...
                for center in TARGET_CENTERS:
                    ...
                    if cache_key not in fair_target_cache:
                        target_model = fit_adapt_filter_fair(target, template, public_config(bank.config), top_k=int(candidate['top_k']), confidence_threshold=float(candidate['confidence_threshold']), regularization=float(candidate['regularization']))
                        fair_target_processed = apply_adapt_filter_fair(target, target_model)
                        fair_target_cache[cache_key] = (target_model, fair_target_processed)
                    started = time.perf_counter()
                    fair_model = fit_adapt_filter_fair(received, template, public_config(bank.config), top_k=int(candidate['top_k']), confidence_threshold=float(candidate['confidence_threshold']), regularization=float(candidate['regularization']))
                    fair_processed = apply_adapt_filter_fair(received, fair_model)
                    ...
                    fair_row = metric_row('FROZEN_REJECTION_CONFIRMATION', 'adapt_filter_fair_frozen', received, target, fair_processed_1d, eval_config, runtime_ms, int(fair_processed.nbytes), {
    ...
    fair_rows = [row for row in rows if row['role'] == 'FROZEN_REJECTION_CONFIRMATION']
    fallback_rows = aggregate(fair_rows, ('jammer', 'jsr_db'))
    oracle_rows = [row for row in aggregate_rows if row['role'] in ('IDENTITY_BASELINE', 'ORACLE_UPPER_BOUND', 'FROZEN_REJECTION_CONFIRMATION')]
    identity_lookup = {(r['jammer'], r['jsr_db']): r for r in aggregate_rows if r['role'] == 'IDENTITY_BASELINE'}
    fair_agg = [r for r in aggregate_rows if r['role'] == 'FROZEN_REJECTION_CONFIRMATION']
    by_jammer_pass = {}
    pass_rows = []
    ...
    final_decision = {
        ...
        'heldout_mode': 'REJECTION_CONFIRMATION_ONLY', 'seed_range': [SEEDS[0], SEEDS[-1]],
    ...
    summary = {
        'task': '036-fix', 'stage': 'stageE', 'status': 'COMPLETED',
        'mode': 'REJECTION_CONFIRMATION_ONLY', 'decision': decision,
        ...
        'heldout_after_tuning': False, 'next_stage': 'stageF',
    }
    ...
    (output_dir / 'heldout_metadata.json').write_text(json.dumps({
        ...
        'no_post_heldout_tuning': True, 'formal_aggregation': 'jammer x JSR across seed x target_position',
